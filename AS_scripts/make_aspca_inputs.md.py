...
def main(args):
    ...
    if len(keep) == 0:
        ...
        ind_order = all_inds[:] # copy the original list so that the in_order below won't add up to it #by dangliu
    else:
        for ind in all_inds:
            if ind in keep:
...
#                out_anc.write(ind + '_A\t' + ind + '_B\t')
    ...
    for ind in all_inds: #by dangliu
        if ind_class[ind] == '0': #by dangliu
            if len(keep) == 0: # so, the admixed ind number won't be exceed the actual one when there is no "keep" argument #by dangliu
                out_adm.write(ind + '_A\t' + ind + '_B\t') #by dangliu
            elif ind in keep: # make keep can be applied to admixed ind too #by dangliu
                out_adm.write(ind + '_A\t' + ind + '_B\t') #by dangliu
        else: #by dangliu
            if len(keep) == 0: #by dangliu
                out_anc.write(ind + '_A\t' + ind + '_B\t') #by dangliu
            elif ind in keep: #by dangliu
                out_anc.write(ind + '_A\t' + ind + '_B\t') #by dangliu
        vit_all[ind + '_A'] = []
        vit_all[ind + '_B'] = []
    out_adm.write('\n')
    ...
    for line in markers:
        ...
        fbk_chunked = chunker(fbk_line, 2) # for 2 ref #by dangliu
        max_chunks = []
...
#            i += 2
        ...
        for ind in all_inds: #this would be a problem because indexing is off #by dangliu
            if len(keep) == 0: #by dangliu
                if ind_class[ind] == '0': #by dangliu
                    adm.append(markers[0][i]) #by dangliu
                    adm.append(markers[0][i+1]) #by dangliu
                elif ind in ind_order: #by dangliu
                    anc.append(markers[0][i]) #by dangliu
                    anc.append(markers[0][i+1]) #by dangliu
            else: #by dangliu
                if ind_class[ind] == '0' and ind in ind_order: #by dangliu
                    adm.append(markers[0][i]) #by dangliu
                    adm.append(markers[0][i+1]) #by dangliu
                elif ind_class[ind] != '0' and ind in ind_order: #by dangliu
                    anc.append(markers[0][i]) #by dangliu
                    anc.append(markers[0][i+1]) #by dangliu
            i += 2 #by dangliu
        i=0
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--keep') # Use a list (IID per line) to keep the individuals want to use for aspca #by dangliu
